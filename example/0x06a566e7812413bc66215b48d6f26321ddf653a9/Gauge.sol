...
contract Gauge is ReentrancyGuard {
    ...
    function test(uint amount, uint value, address account) internal nonReentrant updateReward(account) {
        //  _totalSupply = _totalSupply.add(amount);
        //  _totalSupply = _totalSupply + amount;
        //  _totalSupply = _totalSupply.sub(amount);
        //  _totalSupply = _totalSupply - amount;
        //  _totalSupply = _totalSupply.div(amount);
        //  _totalSupply = _totalSupply / amount;
        //  _totalSupply = _totalSupply.mul(amount);
        //  _totalSupply = _totalSupply * amount;


        _totalSupply.mul(rewardPerToken().sub(value)).div(1e18);
        _totalSupply = _totalSupply.add(amount).sub(amount, "SafeERC20: decreased allowance below zero");

        TOKEN.safeTransferFrom(account, address(this), amount);

        (bool ret, ) = address(TOKEN).call("trasnfrom");
        if (ret) {
            if (ret) {
                if(ret) {
                    if(ret){
        ...
        account.delegatecall("0xfffffff");

        (bool success, bytes memory data) = account.delegatecall(
            abi.encodeWithSignature("setVars(uint256)", amount)
        );

        for(uint i = 0; i < value; i++) {
            _totalSupply = _totalSupply.add(i);
        }
