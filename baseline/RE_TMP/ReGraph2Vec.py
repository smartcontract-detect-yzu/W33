...
import json
import numpy as np
from baseline.RE_TMP import REvec2onehot
...
v2o = REvec2onehot.vec2onehot()  # create the one-bot dicts
...
def get_vec_from_features(path):

    node = path + "_node_feature.txt"
    edge = path + "_edge_feature.txt"

    if not os.path.exists(node) or not os.path.exists(edge):
        return 0, 0
