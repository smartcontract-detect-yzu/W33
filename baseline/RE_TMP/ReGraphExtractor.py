...
def save_feature_to_txt(node_feature, file_name):
    with open(file_name, "w+") as f:

        # 针对每一行单独解析
        for nodes_line in node_feature:
...
VUL_DATASET = "DataSetFinal\\"
VUL_FI_DATASET = "SolidiFIAST"


def create_graph_info_for_contract(file_name):

    if not os.path.exists(file_name):
        print("文件名称：{}".format(file_name))
        raise RuntimeError("缺少智能合约源码文件")

    print("开始解析目标文件.............")
    print("文件名称：{}".format(file_name))

    # TODO:解析合约图中的：节点特征和边特征
    node_feature, edge_feature = generate_graph(file_name)
...
if __name__ == "__main__":
    # file_path = DATASET_PREFIX + VUL_DATASET + "blockInfoDependency\\" + "blockInfoDependency_1_0x0a1386b0c52c5f230083071b22e2864efaefbc9b_affectedByMiners\\"
    # file_name = "0x0a1386b0c52c5f230083071b22e2864efaefbc9b_affectedByMiners.sol"
    # create_graph_info_for_contract(file_path, file_name)

    test_contract = "test\\Reentrance_01.sol"
    ...
    save_feature_to_txt(node_feature, "test\\node_feature.txt")
    save_feature_to_txt(edge_feature, "test\\edge_feature.txt")
