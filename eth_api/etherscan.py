...
class Etherscan:
    ...
    # 国内可连网络
    CN_BASE_URL = "https://api-cn.etherscan.com/api"

    # 需要代理
    BASE_URL = "https://api.etherscan.io/api"
    ROPSTEN_URL = "https://api-ropsten.etherscan.io/api"
    GOERLI_URL = "https://api-goerli.etherscan.io/api"
    KOVAN_URL = "https://api-kovan.etherscan.io/"

    def __init__(
            self, key: str, plan: str = "free", fail_silently: bool = False, vpn_flag: int = 0
    ):
        self.vpn = vpn_flag
        self.key = key
    ...
    def _get(self, params: Optional[Dict[str, Any]] = None) -> Any:
        ...
        if self.vpn:
            url = "{}?module={}&action={}&address={}&apikey={}".format(
                self.GOERLI_URL,
                "contract",
                "getsourcecode",
                params["address"],
            ...
            r = requests.get(url=self.CN_BASE_URL, params=clean_params(params))
